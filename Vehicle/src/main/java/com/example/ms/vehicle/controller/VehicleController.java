...
import com.example.ms.vehicle.Constants;
...
public class VehicleController {
    ...
    public String create(Request request, Response response) {
        Vehicle vehicle = request.getBodyAs(Vehicle.class,
                "Vehicle details not provided");
        ValidationEngine.validateAndThrow(vehicle);
        Vehicle saved = vehicles.create(vehicle);
        ...
        String locationPattern = request.getNamedUrl(HttpMethod.GET,
                Constants.Routes.VEHICLE_INSTANCE);
        response.addLocationHeader(LinkUtils.formatUrl(locationPattern,
                Constants.Url.VEHICLE_ID, saved.getId()));
    ...
    public Vehicle read(Request request, Response response) {
        ...
        String selfUrl = LinkUtils.formatUrl(selfPattern,
                Constants.Url.VEHICLE_ID, vehicle.getId());
    ...
    public LinkableCollection<Vehicle> readAll(Request request, Response response) {
        ...
        // Add 'self' links
        String vehicleSelfPattern = request.getNamedUrl(HttpMethod.GET,
                Constants.Routes.VEHICLE_INSTANCE);

        for (Vehicle result : results) {
            String selfUrl = LinkUtils.formatUrl(vehicleSelfPattern,
                    Constants.Url.VEHICLE_ID, result.getId());
            result.addLink(new Link(RelTypes.SELF, selfUrl));
        }

        String selfUrl = request.getNamedUrl(HttpMethod.GET,
                Constants.Routes.VEHICLE_COLLECTION);
        LinkableCollection<Vehicle> wrapper
                = new LinkableCollection<Vehicle>(results);
